...
def get_param(shape):
    param = mindspore.Parameter(initializer('XavierUniform', shape))
    return param
    
class MEAN_MLP_hym(nn.Cell):
    # self, user_num, country_num, device_num, app_num, class_num, init_dim, max_count, max_time, hist, features_len
    def __init__(self, app_num):
        super().__init__()
        ## scalers init
    ...
    #     max_rate =  (self.MAX_COUNT-batch_hist_counts)/(self.MAX_COUNT-self.MIN_COUNT)
    #     min_rate =  (batch_hist_counts-self.MIN_COUNT)/(self.MAX_COUNT-self.MIN_COUNT)
        
    #     resutl1 = self.batch_mat_mul(max_rate.float(), self.max_count_emb.float())
    #     result2 = self.batch_mat_mul(min_rate.float(), self.min_count_emb.float())
    ...
    #     max_rate =  (self.MAX_TIME-batch_hist_times)/(self.MAX_TIME-self.MIN_TIME)
    #     min_rate =  (batch_hist_times-self.MIN_TIME)/(self.MAX_TIME-self.MIN_TIME)
        
    #     resutl1 = self.batch_mat_mul(max_rate.float(), self.max_time_emb.float())
    #     result2 = self.batch_mat_mul(min_rate.float(), self.min_time_emb.float())
    ...
    def construct(self, stacked_input):
            # self.app_emb[0] = self.zeros_like(self.app_emb[0])s
            # self.class_emb[0] = self.zeros_like(self.class_emb[0])
            ...
            # input_embeddings = self.concat((self.reshape(self.concat(variable_embeddings), (cur_batch_size, -1)), 
            #                     self.reshape(self.concat(sequence_embeddings), (cur_batch_size, -1))))
        input_embeddings = stacked_input
        shared_embeddings = self.Shared_Layer(input_embeddings)
        app_output = self.Final_App_Layer(self.App_Layers(shared_embeddings))
            # time_output = self.Final_Time_Layer(self.Time_Layers(shared_embeddings))
        y_pred = self.sigmoid(app_output)
        return y_pred
